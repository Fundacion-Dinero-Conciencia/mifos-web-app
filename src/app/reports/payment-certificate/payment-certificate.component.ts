...
@Component({
  selector: 'mifosx-payment-certificate',
  templateUrl: './payment-certificate.component.html',
  styleUrls: ['./payment-certificate.component.scss']
})
export class PaymentCertificateComponent implements OnInit, AfterViewInit {
  ...
  downloadReport() {
    this.loading = true;
    this.reportsService
      .getPaymentCertificatePDF(this.reportForm.value.group.id)
      .pipe(
        finalize(() => {
          this.loading = false;
        })
